...
export async function createDailyTestNotification(params: {
  ...
  return createNotification({
    ...
    message: `${params.bitchName} — Day ${params.day}: Last reading was ${params.lastLevel} ng/mL. Daily testing required to catch the breeding window.`,
    actionUrl: `/calculators/progesterone/${params.heatCycleId}`,
    actionLabel: 'Add Reading',
    relatedEntityType: 'heat_cycle',
    relatedEntityId: params.heatCycleId,
...
export async function createBreedingWindowNotification(params: {
  ...
  return createNotification({
    ...
    message: `${params.bitchName} — Day ${params.day}: Progesterone is ${params.progesteroneLevel} ng/mL. ${isOptimal ? 'Perfect for natural breeding or AI!' : 'Optimal for frozen semen AI!'}`,
    actionUrl: `/calculators/progesterone/${params.heatCycleId}`,
...
export async function createOvulationNotification(params: {
  ...
  return createNotification({
    ...
    message: `${params.bitchName} — Ovulation detected on Day ${params.day}. Optimal breeding window is now open.`,
    actionUrl: `/calculators/progesterone/${params.heatCycleId}`,
...
export async function createWhelpingApproachingNotification(params: {
  ...
  return createNotification({
    ...
    message: `${params.bitchName} — Expected whelping date is ${params.whelpingDate} (${params.daysUntil} days away). Prepare whelping area and supplies.`,
